...
if opt.one_step
    % we turn this function inside out. 
end
...
%         for sample = 1:opt.num_hypersamples
%             gp.hypersamples(sample).X_c(evaluation,:) = x;
% 
%             w_0 = ...
%                 logistic(gp.hypersamples(sample).hyperparameters(gp.w0_inds), ...
%                             gp.hypersamples(sample).w_max);
% 
%             gp.hypersamples(sample).w_c(evaluation,:) = w_0;
%             allowed_w_max = min(2*gp.hypersamples(sample).w_c, [], 1);
%             gp.hypersamples(sample) = ...
%                     adjust_w_max(gp.hypersamples(sample), ...
%                     allowed_w_max, gp.w0_inds);
%                 
%            
%             
%         end
    ...
    if ~retrain_now 
        ...
        % retrain gp
        [gp, quad_gp] = ...
            train_spgp(gp, X_data_so_far, y_data_so_far, opt);
