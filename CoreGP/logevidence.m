function [logev,std_logev] = logevidence(covvy)
...
if ((isfield(covvy, 'use_derivatives') && covvy.use_derivatives == true) || (isfield(covvy, 'covfn') && nargin(covvy.covfn)~=1))
    derivs=true;
else
...
widthfrac=0.2; %Change for input scale adjustment for BMC
...
if derivs
    ...
    unc=unc+(det(2*pi*diag(widths.^2+priorSDs.^2)))^(-0.5); %Variance
                                                            %without
                                                            %multiplicative
                                                            %factor h
    
    std_logev = sqrt(unc)*mean(likelihoods)/(nT_invK*likelihoods); %pseudo max likelihood
                                     %estimate of h as mean likelihood
