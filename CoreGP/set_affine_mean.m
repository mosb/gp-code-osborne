function gp = set_affine_mean(gp, XData, YData, num_samples)
% Set up gp with a quadratic prior mean. It can be called in three ways:
% gp = set_planar_mean(gp, num_samples) sets zero mean, 1 SD
% priors for each hyperparameter of the mean function, with num_samples
% taken from each. 
% gp = set_planar_mean(gp, XData, YData) 
...
% gp = set_planar_mean(gp, XData, YData, num_samples) 
...
for ind=1:length(planar_weights)
    gp.hyperparams(numel(gp.hyperparams)+1) = orderfields(...
        struct('name',['PlanarMeanWeight',num2str(ind)],...
        'priorMean',planar_weights(ind),...
        'priorSD',planar_priorSDs(ind),...
...
hps_struct = set_hps_struct(gp);
gp.meanfn = @(flag) affine_mean_fn(hps_struct,flag);